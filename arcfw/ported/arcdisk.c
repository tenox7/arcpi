...
Abstract:
    ...
    Ported verbatim from PRIVATE/NTOS/BOOT/LIB/ARCDISK.C for ARM (the _i386_
    EISA->multi rename block compiles out; the #else strcpy path is used). It
    now works because the ARM firmware emulator synthesizes an ARC config tree
    (arcfw/arm/config.c) that BlConfigurationInitialize copies into
    BlLoaderBlock->ConfigurationRoot, so BlSearchConfigTree finds the disk and
    BlpReadSignature opens "...partition(0)" to read its MBR signature.
