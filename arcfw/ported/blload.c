...

{
    ...
    // ARM/GCC: original is MSVC cast-as-lvalue `*((SHORT UNALIGNED *)TableAddress)++`
    // (read the SHORT, then advance TableAddress by sizeof(SHORT)). GCC rejects a cast
    // as an lvalue, so split it; behaviour is identical.
