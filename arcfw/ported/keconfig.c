//
// KeFindConfigurationEntry + KeFindConfigurationNextEntry, ported verbatim from
// PRIVATE/NTOS/KE/CONFIG.C (the #pragma alloc_text INIT lines are dropped). They
// search the ARC configuration tree that the firmware emulator synthesizes
// (arcfw/arm/config.c) and BlConfigurationInitialize copies into
// BlLoaderBlock->ConfigurationRoot. Both are NULL-root-safe (the while(Child!=NULL)
// loop falls straight through to a NULL return), so osloader.c's early cache lookups
// at lines 274-286 - which run before the tree is built - return NULL exactly as the
// former stub did.
//
#include "bldr.h"
