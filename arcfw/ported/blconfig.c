//
// Curated subset of PRIVATE/NTOS/BOOT/LIB/BLCONFIG.C, ported verbatim for ARM
// (see ARCHITECTURE.md "M1a header method" / memory nt35-port-header-method).
//
// This file provides ONLY the ARC device-name generation slice:
//   BlGenerateDeviceNames  (replaces the stub in stubs.c)
//   BlGetNextToken / BlMatchToken  (its private helpers)
//   the adapter/controller/peripheral token tables + their enums
// All three are pure ARC-pathname string parsing - they do NOT touch the ARC
// configuration tree, so they are safe to use while ConfigurationRoot is still
// NULL (BlConfigurationInitialize stubbed to ESUCCESS).
//
// DELIBERATELY OMITTED from the original BLCONFIG.C (they walk/build the ARC
// configuration tree, which is still deferred - see stubs.c BlConfigurationInitialize
// and the Tier-A config-tree cluster in memory nt35-phase2-kernel-roadmap):
//   BlConfigurationInitialize, BlSearchConfigTree, BlGetPathnameFromComponent,
//   the MnemonicTable.  Bring those in when the firmware emulator synthesizes a
//   real config tree.
//
#include "bldr.h"
#include "ctype.h"
